...
class train_model:
    ...
    def train(self):
        data =pd.read_csv("C:/Users/SHYAM/Projects/Major Projects/Resume Prediction/training_dataset.csv")
        array = data.values
...
def prediction_result(top, aplcnt_name, cv_path, personality_values):
  ...
  #  result.geometry('700x550')
    ...
    result.configure(background='Yellow')
    result.title("Resume based Personality Prediction System")
    
    #Title
    titleFont = font.Font(family='Arial', size=40, weight='bold')
    Label(result, text="Result - Personality Prediction", foreground='Red', bg='Black', font=titleFont, pady=10, anchor=CENTER).pack(fill=BOTH)
    Label(result, text = str('{} : {}'.format("Name:", aplcnt_name.get())).title(), foreground='Red', bg='Black', anchor='w').pack(fill=BOTH)
    Label(result, text = str('{} : {}'.format("Age:", age)), foreground='Red', bg='Black', anchor='w').pack(fill=BOTH)
    for key in data.keys():
        if data[key] is not None:
            Label(result, text = str('{} : {}'.format(check_type(key.title()),check_type(data[key]))), foreground='Red', bg='Black', anchor='w', width=60).pack(fill=BOTH)
    Label(result, text = str("perdicted personality: "+personality).title(), foreground='Red', bg='Black', anchor='w').pack(fill=BOTH)
...
def perdict_person():
    ...
    #Job_Form
    job_list=('Select Job', '101-Developer at Neilson', '102-Chef at La Pinoz', '103-Professor at SRM')
    job = StringVar(top)
    job.set(job_list[0])
    l1=Label(top, text="Applicant Name", foreground='Red', bg='Black').place(x=70, y=130)
    l2=Label(top, text="Age", foreground='Red', bg='Black').place(x=70, y=160)
    l3=Label(top, text="Gender", foreground='Red', bg='Black').place(x=70, y=190)
    l4=Label(top, text="Upload Resume", foreground='Red', bg='Black').place(x=70, y=220)
    l5=Label(top, text="Enjoy New Experience or thing(Openness)", foreground='Red', bg='Black').place(x=70, y=250)
    l6=Label(top, text="How Offen You Feel Negativity(Neuroticism)", foreground='Red', bg='Black').place(x=70, y=280)
    l7=Label(top, text="Wishing to do one's work well and thoroughly(Conscientiousness)", foreground='Red', bg='Black').place(x=70, y=310)
    l8=Label(top, text="How much would you like work with your peers(Agreeableness)", foreground='Red', bg='Black').place(x=70, y=340)
    l9=Label(top, text="How outgoing and social interaction you like(Extraversion)", foreground='Red', bg='Black').place(x=70, y=370)    
    sName=Entry(top)
    ...
    submitBtn=Button(top, padx=2, pady=0, text="Submit", bd=0, foreground='Red', bg='Yellow', font=(12))
...
if __name__ == "__main__":
    ...
    lab=Label(root, text="Personality Prediction System", bg='Black', font=titleFont, pady=30).pack()
    b2=Button(root, padx=4, pady=4, width=30, text="Predict Personality", bg='Black', foreground='Red', bd=1, font=homeBtnFont, command=perdict_person).place(relx=0.5, rely=0.5, anchor=CENTER)
    root.mainloop()